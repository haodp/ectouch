...
namespace app\kernel\template\taglib;

use app\kernel\template\TagLib;
...
class Cx extends Taglib
{
    ...
    public function tagVolist($tag, $content)
    {
        ...
        $parseStr .= 'if(is_array(' . $name . ') || ' . $name . ' instanceof \app\kernel\Collection || ' . $name . ' instanceof \app\kernel\Paginator): $' . $key . ' = 0;';
    ...
    public function tagForeach($tag, $content)
    {
        ...
        $parseStr .= 'if(is_array(' . $name . ') || ' . $name . ' instanceof \app\kernel\Collection || ' . $name . ' instanceof \app\kernel\Paginator): ';
    ...
    public function tagEmpty($tag, $content)
    {
        $name     = $tag['name'];
        $name     = $this->autoBuildVar($name);
        $parseStr = '<?php if(empty(' . $name . ') || ((' . $name . ' instanceof \app\kernel\Collection || ' . $name . ' instanceof \app\kernel\Paginator ) && ' . $name . '->isEmpty())): ?>' . $content . '<?php endif; ?>';
