...
use app\kernel\db\exception\BindParamException;
use app\kernel\db\exception\DataNotFoundException;
use app\kernel\db\exception\ModelNotFoundException;
use app\kernel\Exception;
use app\kernel\exception\DbException;
use app\kernel\exception\PDOException;
use app\kernel\Loader;
use app\kernel\Model;
use app\kernel\model\Relation;
use app\kernel\model\relation\OneToOne;
use app\kernel\Paginator;
...
class Query
{
    ...
    public function __call($method, $args)
    {
        if (isset(self::$extend[strtolower($method)])) {
            ...
            $name = parse_name(substr($method, 7));
            array_unshift($args, $name);
            return call_user_func_array([$this, 'whereOr'], $args);
        } elseif (strtolower(substr($method, 0, 5)) == 'where') {
            $name = parse_name(substr($method, 5));
            array_unshift($args, $name);
    ...
    public function paginate($listRows = null, $simple = false, $config = [])
    {
        ...
        /** @var Paginator $class */
        $class = false !== strpos($config['type'], '\\') ? $config['type'] : '\\app\\kernel\\paginator\\driver\\' . ucwords($config['type']);
    ...
    protected function withAggregate($relation, $aggregate = 'count', $field = '*', $subQuery = true)
    {
        ...
        if (!$subQuery) {
            ...
            foreach ($relations as $key => $relation) {
                ...
                $this->field([$count => parse_name($relation) . '_' . $aggregate]);
            }
        }
