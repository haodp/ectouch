...
namespace app\kernel\model\concern;

use app\kernel\Collection;
use app\kernel\db\Query;
use app\kernel\Loader;
use app\kernel\Model;
use app\kernel\model\Relation;
use app\kernel\model\relation\BelongsTo;
use app\kernel\model\relation\BelongsToMany;
use app\kernel\model\relation\HasMany;
use app\kernel\model\relation\HasManyThrough;
use app\kernel\model\relation\HasOne;
use app\kernel\model\relation\MorphMany;
use app\kernel\model\relation\MorphOne;
use app\kernel\model\relation\MorphTo;
...
trait RelationShip
{
    ...
    {
        // 检测修改器
        $method = 'set' . parse_name($name, 1) . 'Attr';
    ...
    public function relationQuery($relations)
    {
        ...
        foreach ($relations as $key => $relation) {
            ...
            $method = parse_name($relation, 1, false);
    ...
    public function eagerlyResultSet(&$resultSet, $relation)
    {
        ...
        foreach ($relations as $key => $relation) {
            ...
            $relation = parse_name($relation, 1, false);
    ...
    public function relationCount(&$result, $relations, $aggregate = 'sum', $field = '*')
    {
        foreach ($relations as $key => $relation) {
            ...
            if (!isset($name)) {
                $name = parse_name($relation) . '_' . $aggregate;
            }
    ...
    public function belongsTo($model, $foreignKey = '', $localKey = '')
    {
        ...
        $relation   = parse_name($trace[1]['function']);
    ...
    public function belongsToMany($model, $table = '', $foreignKey = '', $localKey = '')
    {
        // 记录当前关联信息
        $model      = $this->parseModel($model);
        $name       = parse_name(basename(str_replace('\\', '/', $model)));
        $table      = $table ?: parse_name($this->name) . '_' . $name;
    ...
    public function morphOne($model, $morph = null, $type = '')
    {
        ...
        if (is_null($morph)) {
            $trace = debug_backtrace(false, 2);
            $morph = parse_name($trace[1]['function']);
        }
    ...
    protected function parseModel($model)
    {
        if (false === strpos($model, '\\')) {
            ...
            array_pop($path);
            array_push($path, parse_name($model, 1));
            $model = implode('\\', $path);
        }
    ...
    {
        $relation = parse_name($attr, 1, false);
    ...
    {
        foreach ($this->relationWrite as $name => $val) {
            $method = parse_name($name, 1, false);
