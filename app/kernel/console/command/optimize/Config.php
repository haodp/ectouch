...
use app\kernel\console\input\Argument;
...
class Config extends Command
{
    ...
    protected function buildCacheContent($module)
    {
        ...
        if (is_file($path . 'tags.php')) {
            ...
            if (is_array($tags)) {
                $content .= PHP_EOL . '\app\kernel\facade\Hook::import(' . (var_export($tags, true)) . ');' . PHP_EOL;
            }
        }
        ...
        if ('' == $module) {
            ...
            if (is_file($path . 'middleware.php')) {
                ...
                if (is_array($middleware)) {
                    $content .= PHP_EOL . '\app\kernel\Container::get("middleware")->import(' . var_export($middleware, true) . ');' . PHP_EOL;
                }
            }
        }
        ...
        if (is_file($path . 'provider.php')) {
            ...
            if (is_array($provider)) {
                $content .= PHP_EOL . '\app\kernel\Container::getInstance()->bind(' . var_export($provider, true) . ');' . PHP_EOL;
            }
        }

        $content .= PHP_EOL . '\app\kernel\facade\Config::set(' . var_export($config->get(), true) . ');' . PHP_EOL;
